...
TEST_CASE("process::resume success", "[process]") {
    ...
    {
        auto target = process::launch("build/test/targets/run_endlessly",
                false);
        auto proc = process::attach(target->pid());
        proc->resume();
        auto status = get_process_status(proc->pid());
...
TEST_CASE("Write register works", "[register]") {
    ...
    auto proc = process::launch(
        "build/test/targets/reg_write", true, channel.get_write());
    channel.close_write();
...
TEST_CASE("Read register works", "[register]") {
    auto proc = process::launch("build/test/targets/reg_read");
    auto& regs = proc->get_registers();
...
TEST_CASE("Reading and writing memory works", "[memory]") {
    bool close_on_exec = false;
    sdb::pipe channel(close_on_exec);
    auto proc = process::launch("build/test/targets/memory", true,
            channel.get_write());
    channel.close_write();
...
TEST_CASE("Hardware breakpoint evades memory checksums", "[breakpoint]") {
    ...
    sdb::pipe channel(close_on_exec);
    auto proc = process::launch(
        "build/test/targets/anti_debugger", true, channel.get_write());
    channel.close_write();
...
TEST_CASE("Watchpoint detects read", "[watchpoint]") {
    ...
    auto& watch = proc->create_watchpoint(
        func, sdb::stoppoint_mode::read_write, 1);
    watch.enable();
    ...
    REQUIRE(
        to_string_view(channel.read()) == "Putting sardines on pizza...\n");
}
